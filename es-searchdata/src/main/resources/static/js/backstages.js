...
$("#search-content").on("keypress", function (event) {
    if (event.keyCode == "13") {
        if ($(this).val().length === 0) {
            ...
            window.location.href = "./result?q=" + encodeURIComponent($(this).val());
        }
    }
})
//处理手气不错返回结果
...
$("#good-luck").on("click",function () {
    ...
    if(searchContent.length === 0 ){
        ...
        $.ajax({
            type : "post",
            url : " http://www.telecredit.cn/api/luck/query",
            data:{domain:searchContent},
            success : function (data) {
                goodLuck(data.domain);
            },
            error : function (data) {
                badLuck();
            }
